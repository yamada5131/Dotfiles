...
def show_menu():
    ...
    graphics_option = questionary.select(
        "Graphics?", choices=["NVIDIA", "Open Source (AMD/Intel/Nouveau)"]
    ).ask()

    keyboard_option = questionary.select(
        "Keyboard layout?", choices=["US", "LATAM"]
    ).ask()
...
def install_packages():
    ...
    package_choice = questionary.select(
        "Install needed packages?", choices=["Yes", "No"]
    ).ask()
...
def install_tpm_plugins():
    title()
    tpm_choice = questionary.select("Install TPM plugins?", choices=["Yes", "No"]).ask()
    ...
    if tpm_choice == "Yes":
        ...
        if os.path.isfile(script_path):
            ...
            console.print(
                f"Error: El script no existe en la ruta {script_path}", style="bold red"
            )
    else:
...
def main():
    ...
    if graphics_option == "NVIDIA":
        ...
        copy_config(
            "./options/nvidia-dummy.conf", "./dots/.config/hypr/source/nvidia.conf"
        )

    keyboard_file = "us.conf" if keyboard_option == "US" else "latam.conf"
    copy_config(
        f"./options/{keyboard_file}", "./dots/.config/hypr/source/keyboard.conf"
    )
    ...
    subprocess.run(
        ["matugen", "image", "./example_wallpaper.jpg"],
        check=True,
